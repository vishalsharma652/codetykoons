    ...
    <>
      <nav className={`fixed top-0 left-0 right-0 z-[50] py-4 transition-all duration-300 bg-[#070714]/92 backdrop-blur-md border-b border-[#6366f1]/15 ${scrolled ? 'py-[10px] bg-[#070714]/98 border-[#6366f1]/25 shadow-[0_4px_30px_rgba(0,0,0,0.4)]' : ''}`}>
        <div className="max-w-[1200px] mx-auto px-8 flex items-center justify-between">
          ...
          {/* Logo */}
          <button onClick={() => handleNav('home')} className="flex items-center gap-[10px] bg-none border-none cursor-pointer p-0 font-inherit" aria-label="Go to Home">
          ...
          <ul className="hidden md:flex items-center gap-8 list-none m-0 p-0">
            ...
            <li>
              <button
                className="bg-gradient-to-r from-[#6366f1] to-[#a855f7] text-white px-[22px] py-[8px] rounded-full border-none cursor-pointer font-inherit font-semibold text-[14px] transition-all duration-300 shadow-[0_0_20px_rgba(99,102,241,0.4)] hover:shadow-[0_0_35px_rgba(99,102,241,0.6)] hover:-translate-y-[2px] whitespace-nowrap"
      ...
      <ul className={`fixed top-0 left-0 bottom-0 w-[75%] max-w-[280px] flex flex-col bg-[#070714]/98 backdrop-blur-lg p-8 pt-20 transition-transform duration-300 ease-in-out border-r border-[#6366f1]/20 shadow-[6px_0_30px_rgba(0,0,0,0.4)] z-[999] list-none md:hidden ${menuOpen ? 'translate-x-0 pointer-events-auto' : '-translate-x-full pointer-events-none'}`}>
        {navLinks.map((link) => (
          <li key={link.id} className="w-full border-b border-white/5">
            <button
              className="block w-full py-4 text-left text-[16px] text-white/75 hover:text-white bg-none border-none font-inherit cursor-pointer"
              onClick={() => handleNav(link.id)}
            >
              {link.label}
            </button>
        ...
        <li className="w-full mt-6">
          <button
            className="w-full bg-gradient-to-r from-[#6366f1] to-[#a855f7] text-white py-3 rounded-full border-none cursor-pointer font-inherit font-semibold text-center shadow-[0_0_20px_rgba(99,102,241,0.4)]"
            onClick={() => handleNav('contact')}
          >
            Get Started
          </button>
